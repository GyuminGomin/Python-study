...
# 09. 전방 탐색
"""
아래의 예시에서 http: 에서 :를 빼고 http만 출력을 하고 싶다고 한다.
    >>> p = re.compile(".+:")
    >>> m = p.search("http://google.com")
    >>> print(m.group())
    http:
위의 예시에서 엄청 복잡한 정규식이여서 그루핑을 추가로 할 수 없다고 가정했을 때, 전방 탐색
확장 구문 사용
- 긍정적 전방 탐색 (?=...) -> ... 에 해당되는 정규식과 매치되어야 하며 조건이 통과되어도
문자열이 소비되지 않는다.
- 부정적 전방 탐색 (?!...) -> ... 에 해당되는 정규식과 매치되지 않아야 하며 조건이 통과
되어도 문자열이 소비되지 않는다.
"""
...
기존 정규식과 검색에서는 동일한 효과를 발휘하지만 : 에 해당하는 문자열이 정규식 엔진에
의해 소비되지 않아(검색에는 포함되지만 검색 결과에서는 제외됨) 검색 결과에서는 : 이 제거
된 후 돌려주는 효과가 있다.
...
위 정규식은 foo.bar, autoexec.bat, sendmail.cf같은 형식의 파일과 매치
위 정규식에 확장자가 "bat인 파일은 제외해야 한다."라는 조건을 추가해 보면,
    .*[.]([^b].?.?|.[^a]?.?|..?[^t]?)$
위 정규식은 확장자가 3개일때만 성립되고, 2개 일때는 만족시키지 못한다.
여기서 exe 파일도 제외해야 한다는 조건이 추가로 생긴다면, 여기서 필요한 게 부정형 전방 탐색
    .*[.](?!bat$).*$
엄청 간단하게 변한다. 확장자가 bat가 아닌 경우에만 통과된다는 의미
bat 문자열이 있는지 조사하는 과정에서 문자열이 소비되지 않으므로 bat가 아니라고 판단되면
그 이후 정규식 매치가 진행된다.
    .*[.](?!bat$|exe$).*$ -> exe파일또한 제외하라는 조건이 추가된다면
"""
...
- 첫번째 매개변수 : "바꿀 문자열(replacement)
- 두번째 매개변수 : "대상 문자열"

만약 딱 한 번만 바꾸고 싶다면, 바꾸기 횟수를 제어할 수 있다.
    >>> p.sub('colour', 'blue socks and red shoes', count=1)
    'colour socks and red shoes'
sub 메서드 말고 subn메서드가 있다.
    >>> p = re.compile('(blue|white|red)')
    >>> p.subn( 'colour', 'blue socks and red shoes')
    ('colour socks and colour shoes', 2)
튜플로 바꿔주고 두번째 요소는 바뀐 횟수를 의미한다.
"""
...
"""
    ...
    010-1234-1234 park
이름 전화번호 --> 전화번호 이름 으로 바꾸어 줌
바꿀 문자열 부분에 \g<그룹이름> 을 사용하면 정규식의 그룹이름 참조 가능
    >>> p = re.compile(r"(?P<name>\w+)\s+(?P<phone>(\d+)[-]\d+[-]\d+)")
...
<.*> 정규식의 매치 결과로 <html><head><title>Title</title> 문자열을 모두 소비해버림
어떻게 하면 Greedy함을 제한하고 <html> 문자열까지만 소비하도록 막을 수 있을까?

다음과 같이 non-greedy 문자인 ? 를 사용하면 * 의 탐욕을 제한할 수 있다.
    >>> print(re.match('<.*?>', s).group())
    <html>
non-greedy 문자인 ?는 *?, +?, ??, {m,n}?와 같이 사용할 수 있다. 가능한 한 가장
최소한의 반복을 수행하도록 도와주는 역할을 한다.
"""